    ...
    try:
        ...
        if rank == 0 and args_dict.get("validation_file_path"):
            val_dataloader = create_pytorch_dataloader(
                [args_dict["validation_file_path"]], batch_size * 2, num_samples=0, is_training=False
            )
        ...
        optimizer = optim.AdamW(model.parameters(), lr=args_dict.get("learning_rate", 0.002))
        ...
        for epoch in range(max_epochs):
            ...
            model.train()
            for i, (features, labels, weights) in enumerate(itertools.islice(train_dataloader, steps_per_epoch)):
            ...
            if rank == 0:
                ...
                val_loss, val_acc, _, _ = evaluate_loop(model_to_eval, val_dataloader, eval_criterion, device)
                ...
                if val_acc > best_val_acc:
                    ...
                    logger.info(f"New best val_acc: {best_val_acc:.4f}. Saving model...")
                    torch.save(model_to_eval.state_dict(), f"/tmp/best_{model_type}_model.pth")
                
                # Use the correct HDFS output path for saving artifacts
                plot_and_save_history(**history, output_dir=final_output_dir, model_type=model_type)

        # --- Final Upload (on rank 0) ---
        if rank == 0 and os.path.exists(f"/tmp/best_{model_type}_model.pth"):
            # Use the correct HDFS output path for final model upload
            upload_local_directory_to_hdfs("/tmp/", final_output_dir)
