...
class ParquetStreamingDataset(IterableDataset):
    ...
    def _init_filesystem(self):
        ...
        try:
            ...
            print(
                f"[DATASET_ERROR] Failed to initialize filesystem for URI '{uri}': {e}"
            )
    ...
    def __iter__(self):
        ...
        iterator = (
            itertools.cycle(self.file_paths)
            if self.is_training
            else iter(self.file_paths)
        )
        ...
        for file_path in iterator:
            try:
                ...
                if self.is_training:
                    print(
                        f"[DATASET_WARNING] Skipping problematic file {file_path}: {e}"
                    )
                    continue
                else:
                    print(
                        f"[DATASET_ERROR] Failed to read evaluation file {file_path}: {e}"
                    )
